...
def make_weekly_csv():
    ...
    # [변수명 체크 포인트] 가장 최신 파일과, 영업일 기준 일주일 전(5번째 전 파일) 비교
    this_file_dt, this_file_path = file_list[0]
    
    target_index = min(5, len(file_list) - 1) # 5일 전 파일 타겟팅
    ...
    # 🌟 [날짜 보정 영역 변수 통일] 
    # 최신 파일 수집 날짜(this_file_dt)의 요일을 확인 (5=토요일, 6=일요일)
    weekday_num = this_file_dt.weekday()
    label_dt = this_file_dt

    if weekday_num == 5:     # 만약 토요일이라면 하루를 빼서 금요일로 변경
        label_dt = this_file_dt - pd.Timedelta(days=1)
    elif weekday_num == 6:   # 만약 일요일이라면 이틀을 빼서 금요일로 변경
        label_dt = this_file_dt - pd.Timedelta(days=2)

    # 기준시각 라벨 업데이트 (트리맵 상단 제목 연동 시 토요일 대신 금요일 날짜가 뜨도록 보정)
    weekly_label = f"{last_file_dt.strftime('%m.%d')}~{label_dt.strftime('%m.%d')} Weekly"
